...
public class MaxCounters {
	...
	private static int[] solution(int N, int[] A) {
		...
		int maxCounter = 0, lastFilledMaxCounter = 0;

		for (int operation : A) {
			if (operation == N + 1) {
				lastFilledMaxCounter = maxCounter;
			} else {
				result[operation - 1] = Math.max(result[operation - 1], lastFilledMaxCounter) + 1;
				maxCounter = Math.max(maxCounter, result[operation - 1]);
			}
		}

		for (int i = 0; i < result.length; i++) {
			if (result[i] < lastFilledMaxCounter)
				result[i] = lastFilledMaxCounter;
		}
