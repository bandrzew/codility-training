/*
    ...
    The DNA sequence is given as a non-empty string S = S[0]S[1]...S[N-1] consisting of N characters. There are M queries, which are given in non-empty arrays P and Q,
    each consisting of M integers. The K-th query (0 ≤ K < M) requires you to find the minimal impact factor of nucleotides contained in the DNA sequence
...
class GenomicRangeQuery {
    ...
    private static final Map<Character, Integer> IMPACT_FACTORS = Map.of(
            'A', 1,
            'C', 2,
            'G', 3,
            'T', 4
    ...
    public static int[] solution(String S, int[] P, int[] Q) {
        ...
        for (int K = 0; K < M; K++) {
            result[K] = 4; //max possible factor
            for (int i = P[K]; i <= Q[K]; i++) {
                int impactFactor = IMPACT_FACTORS.get(S.charAt(i));
                result[K] = Math.min(impactFactor, result[K]);
                if (result[K] == 1) {
                    break;
                }
            }
